...
describe('Recur Examples', function () {
  describe('instances', function () {
    ...
    it('At 11:30am on March 21, 2013 starting past the date should not occur', function () {
      ...
      const start = new Date('2013-03-31T03:05:23Z');
      const end = new Date('2013-02-21T03:40:10Z');
      const expected = later.NEVER;
    ...
    it('Every second Tuesday at 4AM and 10PM', function () {
      ...
      const start = new Date('2013-02-01T00:00:00Z');
      const end = new Date('2013-05-01T00:00:00Z');
      const expected = [
    ...
    it('Once a month on the closest weekday to the 15th', function () {
      ...
      const start = new Date('2013-03-01T00:00:00Z');
      const end = new Date('2013-10-01T00:00:00Z');
      const expected = [
      ...
      const next = later.schedule(sched).next(7, start, end);
      next.should.eql(expected);
    ...
    it('Next time Jan 1st is ISO Week #53', function () {
      ...
      const start = new Date('2013-03-01T00:00:00Z');
      const end = new Date('2016-10-01T00:00:00Z');
      const expected = new Date('2016-01-01T00:00:00Z');
    ...
    it('Every minute except multiples of 2 and 3', function () {
      ...
      const start = new Date('2013-03-01T00:00:00Z');
      const end = new Date('2013-03-01T00:18:00Z');
      const expected = [
    ...
    it('Last day of every month', function () {
      ...
      const sched = later.parse.recur().last().dayOfMonth();

      const start = new Date('2012-01-01T00:00:00Z');
      const end = new Date('2013-01-01T00:00:00Z');
      const expected = [
      ...
      const next = later.schedule(sched).next(12, start, end);
      next.should.eql(expected);
    ...
    it('All 31st numbered days of the year', function () {
      ...
      const sched = later.parse.recur().on(31).dayOfMonth();

      const start = new Date('2013-01-01T00:00:00Z');
      const end = new Date('2014-01-01T00:00:00Z');
      const expected = [
    ...
    it('Every Friday the 13th', function () {
      ...
      const sched = later.parse.recur().on(13).dayOfMonth().on(6).dayOfWeek();
    ...
    it('should recur everyday except on weekends', function () {
      ...
      const start = new Date('2012-01-05T00:00:00Z');
      const end = new Date('2012-01-11T10:00:00Z');
      const expected = [
    ...
    it('should recur Wednesday every 4 weeks at 8am starting on the 5th week', function () {
      ...
      const start = new Date('2012-01-01T23:59:15Z');
      const end = new Date('2012-06-21T08:00:00Z');
      const expected = [
    ...
    it('should find the first second of every month', function () {
      ...
      const start = new Date('2012-01-01T00:00:00Z');
      const end = new Date('2012-12-02T00:00:00Z');
      const expected = [
    ...
    it('should find the last second of every month', function () {
      ...
      const start = new Date('2012-01-01T00:00:00Z');
      const end = new Date('2013-01-05T23:59:59Z');
      const expected = [
  ...
  describe('ranges', function () {
    it('(1) After 15 minutes and before 30 minutes except multiples of 5 and 8', function () {
      ...
      const start = new Date('2013-03-21T03:06:23Z');
      const end = new Date('2013-03-21T04:10:45Z');
      const expected = [
      ...
      const next = later.schedule(sched).nextRange(10, start, end);
      next.should.eql(expected);

      const previous = later.schedule(sched).prevRange(10, end, start);
      previous.should.eql(expected.reverse());
    ...
    it('(2) After 15 minutes and before 30 minutes except multiples of 5 and 8', function () {
      ...
      const start = new Date('2013-03-21T03:22:00Z');
      const end = new Date('2013-03-21T04:07:30Z');
      const expected = [
      ...
      const next = later.schedule(sched).nextRange(8, start, end);
      next.should.eql(expected);

      const previous = later.schedule(sched).prevRange(8, end, start);
      previous.should.eql(expected.reverse());
    ...
    it('After 30 minutes except seconds multiples of 5', function () {
      ...
      const start = new Date('2013-03-21T03:05:23Z');
      const end = new Date('2013-03-21T10:40:10Z');
    ...
    it('Every second after 30 minutes except seconds multiples of 5', function () {
      ...
      const start = new Date('2013-03-21T03:05:23Z');
      const end = new Date('2013-03-21T03:30:15Z');
      const expected = [
      ...
      const next = later.schedule(sched).nextRange(3, start, end);
      next.should.eql(expected);

      const previous = later.schedule(sched).prevRange(3, end, start);
      previous.should.eql(expected.reverse());
    ...
    it('First 5 minutes of every hour', function () {
      ...
      const sched = later.parse.recur().on(0, 1, 2, 3, 4).minute();
      ...
      const next = later.schedule(sched).nextRange(7, start, end);
      next.should.eql(expected);

      const previous = later.schedule(sched).prevRange(7, end, start);
      previous.should.eql(expected.reverse());
    ...
    it('Every weekday between 8:30AM and 5:30PM', function () {
      ...
      const start = new Date('2013-01-01T00:00:00Z');
      const end = new Date('2013-01-10T00:00:00Z');
      const expected = [
    ...
    it('Every weekend between 8AM and 5PM', function () {
      ...
      const start = new Date('2013-02-01T00:00:00Z');
      const end = new Date('2013-02-24T00:00:00Z');
      const expected = [
