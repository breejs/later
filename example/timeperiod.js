...
later.partOfDay = later.pd = {
  ...
  val(d) {
    return later.h.val(d) < 12 ? 0 : later.h.val(d) < 18 ? 1 : 2;
  },

  // use val(d) to determine if a particular value is valid
  isValid(d, value) {
    return later.pd.val(d) === value;
  },
  ...
  // are in
  start(d) {
    const hour = later.pd.val(d) === 0 ? 0 : later.pd.val(d) === 1 ? 12 : 18;
  ...
  // is the last second of the part of the day we are in
    ...
    const hour = later.pd.val(d) === 0 ? 11 : later.pd.val(d) === 1 ? 5 : 23;
  ...
  next(d, value) {
    const hour = value === 0 ? 0 : value === 1 ? 12 : 18;
  ...
  prev(d, value) {
    const hour = value === 0 ? 11 : value === 1 ? 5 : 23;
...
const next = later.schedule(sched).next(5, new Date(2013, 3, 21));
